...
#define FLEXUS_DECLARE_SIMULATOR(Name)                                                             \
  ...
  volatile const char theSimulatorIDString[] __attribute__((section("qflex_version"))) =           \
      "QFlex simulator. Built as " Name " from commit hash " FLEXUS_GIT_HASH                       \
      " " FLEXUS_GIT_TREE_STATUS " from branch " FLEXUS_GIT_BRANCH;                                \
  }                                                                                                \
